...
class CNCGCodeGenerator:
    ...
    def setup_gui(self):
        ...
        # Retract Height (Z-axis retract height)
        ttk.Label(params_frame, text="Retract Height (Z retract):").grid(row=3, column=0, sticky=tk.W, pady=5)
        self.retract_height_var = tk.DoubleVar(value=20.0)
        self.retract_height_entry = ttk.Entry(params_frame, textvariable=self.retract_height_var, width=15)
        self.retract_height_entry.grid(row=3, column=1, sticky=(tk.W, tk.E), padx=(10, 0), pady=5)
        
        # Cut Depth (Z-axis cutting depth)
        ttk.Label(params_frame, text="Cut Depth (Z cut):").grid(row=4, column=0, sticky=tk.W, pady=5)
        ...
        self.cut_depth_entry.grid(row=4, column=1, sticky=(tk.W, tk.E), padx=(10, 0), pady=5)
        
        # Plunge Feedrate
        ttk.Label(params_frame, text="Plunge Feedrate:").grid(row=5, column=0, sticky=tk.W, pady=5)
        ...
        self.plunge_feedrate_entry.grid(row=5, column=1, sticky=(tk.W, tk.E), padx=(10, 0), pady=5)
        
        # Cut Feedrate
        ttk.Label(params_frame, text="Cut Feedrate:").grid(row=6, column=0, sticky=tk.W, pady=5)
        ...
        self.cut_feedrate_entry.grid(row=6, column=1, sticky=(tk.W, tk.E), padx=(10, 0), pady=5)
        
        # Start Y position
        ttk.Label(params_frame, text="Start Y Position:").grid(row=7, column=0, sticky=tk.W, pady=5)
        self.start_y_var = tk.DoubleVar(value=0.0)
        self.start_y_entry = ttk.Entry(params_frame, textvariable=self.start_y_var, width=15)
        self.start_y_entry.grid(row=7, column=1, sticky=(tk.W, tk.E), padx=(10, 0), pady=5)
        
        # End Y position
        ttk.Label(params_frame, text="End Y position:").grid(row=8, column=0, sticky=tk.W, pady=5)
        self.end_y_var = tk.DoubleVar(value=3320.0)
        self.end_y_entry = ttk.Entry(params_frame, textvariable=self.end_y_var, width=15)
        self.end_y_entry.grid(row=8, column=1, sticky=(tk.W, tk.E), padx=(10, 0), pady=5)
    ...
    def generate_code(self):
        ...
        try:
            ...
            retract_height = self.retract_height_var.get()
